...
let direcrtDataProducerId;

let sockList = {};  // --- key: Websocket Id, value: {"producerId": transport.id, "consumerId": transport.id}
...
httpsServer.listen(port, () => {
    ...
    console.log('https://' + ip_addr + ':' + port + '/rtc_client.html');
})
...
io.on('connection', sock => {
    ...
    sock.on('getRtpCapabilities', (_, callback) => {
        ...
        console.log("Total List: " + Object.keys(sockList).length);
        if (Object.keys(sockList).length > limitClient) {
    ...
    sock.on('createProducerTransport', async (_, callback) => {
        ...
        sockList[sock.id]["producerId"] = transport.id;
    });
    ...
    sock.on('createConsumerTransport', async (_, callback) => {
        ...
        sockList[sock.id]["consumerId"] = transport.id;
    });
    ...
    // --- use direcrtDataProducerId
    sock.on('consumeData', async (req, callback) => {
        const transport = consumerList[req.transportId];
        const dataConsumer = await transport.consumeData({ dataProducerId: direcrtDataProducerId, });
        const params = {
            id: dataConsumer.id,
    ...
    sock.on("disconnect", () => {
        ...
        //console.log(JSON.stringify(sockList[sock.id]));

        const producerId = sockList[sock.id]["producerId"];
        const consumerId = sockList[sock.id]["consumerId"];
...
async function createDirectProducer() {
    ...
    //console.log("direcrtDataProducerId: " + direcrtDataProducerId);
    setInterval(() => {
        screenshot().then((img) => {
